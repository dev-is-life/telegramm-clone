		...
		<>
			<div className='flex items-center justify-between relative'>
				...
				<Popover open={isNotification} onOpenChange={setIsNotification}>
					...
					<PopoverContent className='w-80 absolute -right-12 dark:bg-background'>
						<div className='flex flex-col space-y-1'>
							{SOUNDS.map(sound => (
								<div
									className={cn(
										'flex justify-between items-center bg-secondary cursor-pointer hover:bg-white/10',
										selectedSound === sound.value && 'bg-white/10',
										session?.currentUser?.sendingSound ===  sound.value && "bg-white/10"
									)}
									key={sound.label}
			...
			<div className='flex items-center justify-between relative'>
				...
				<Popover open={isSounding} onOpenChange={setIsSounding}>
					...
					<PopoverContent className='w-80 absolute -right-14 bg-background'>
						<div className='flex flex-col space-y-1'>
							{SOUNDS.map(sound => (
								<div
									className={cn(
			...
			<div className='flex items-center justify-between relative'>
				...
				<Switch
					className='bg-red-800 text-red-600 border-none'
